...
const KindergartenCard = ({ kindergarten, onViewDetails, onRegister, onBook }: KindergartenCardProps) => {
  ...
  const serviceNames: Record<string, string> = {
    ...
    languages: t('language.select'), // Placeholder if no specific key
    quran: t('filter.extracurricular'), // Placeholder
  ...
  return (
    <Card className="group overflow-hidden bg-card border-border hover:shadow-hover transition-all duration-500 rounded-2xl">
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        {/* Municipality */}
        <div className={`absolute bottom-3 ${dir === 'rtl' ? 'right-3' : 'left-3'} flex items-center gap-1 px-2.5 py-1 bg-card/90 backdrop-blur-sm rounded-full`}>
      ...
      <CardContent className="p-5">
        {/* Name */}
        <h3 className={`text-xl font-bold text-foreground mb-2 group-hover:text-primary transition-colors ${dir === 'rtl' ? 'text-right' : 'text-left'}`}>
          {name}
        </h3>

        {/* Address */}
        <p className={`text-sm text-muted-foreground mb-4 line-clamp-1 ${dir === 'rtl' ? 'text-right' : 'text-left'}`}>
          {address}
        </p>
        ...
        <div className={`flex gap-2`}>
          ...
          <Button
            className="flex-1 rounded-xl gradient-accent border-0 shadow-soft hover:shadow-hover transition-all duration-300 text-primary-foreground font-bold"
